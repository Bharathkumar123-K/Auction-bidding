...
from fixed_admin_items import create_admin_items_endpoint
...
try:
    ...
    # Create the fixed admin items endpoint
    create_admin_items_endpoint(app, items_collection)
    print("Created fixed admin items endpoint")
...
# Utility function to serialize MongoDB documents for JSON
...
def login():
    print(f'Login endpoint called with method: {request.method}')  # Debug log

    if request.method == 'OPTIONS':
        # Handle CORS preflight request
        print('Handling CORS preflight request')  # Debug log
    ...
    try:
        print('Processing POST login request')  # Debug log
        data = request.get_json()
        ...
        if user:
            ...
            print(f'Password check: {check_password_hash(user["password_hash"], data["password"])}')  # Debug log
...
def update_profile():
    try:
        ...
        profile_fields = [
            ...
            'occupation', 'bio', 'preferred_categories',
        ...
        print(f"Update data to be saved: {update_data}")
        ...
        if update_data:
            ...
            print(f"Profile update result: {result.modified_count} document(s) modified")
...
@jwt_required()
def get_admin_items():
    print("=== NEW ADMIN ITEMS ENDPOINT CALLED ===")
    try:
        user_id = get_jwt_identity()
        ...
        for item in items:
            try:
                # Manual serialization to avoid any issues
                serialized = {
                    'id': str(item['_id']),  # Use 'id' instead of '_id' for consistency
                    '_id': str(item['_id']),  # Keep _id for backward compatibility
                    'name': item.get('name', ''),
                ...
                if 'end_time' in item and item['end_time']:
                    serialized['end_time'] = item['end_time'].isoformat()
                else:
                    serialized['end_time'] = None

                if 'created_at' in item and item['created_at']:
                    serialized['created_at'] = item['created_at'].isoformat()
                else:
                    serialized['created_at'] = None

                if 'seller_id' in item and item['seller_id']:
                    serialized['seller_id'] = str(item['seller_id'])
                else:
                    serialized['seller_id'] = None

                result.append(serialized)
                print(f"Serialized item: {serialized['name']}")
...
@app.route('/api/admin/select-winner', methods=['POST'])
@jwt_required()
def select_winner():
    try:
        ...
        if not all([item_id, winner_user_id, winning_amount]):
            return jsonify({'error': 'Missing required fields'}), 400

        # Update the item with winner information
        items_collection.update_one(
            {'_id': ObjectId(item_id)},
            {
                '$set': {
                    'winner_user_id': ObjectId(winner_user_id),
                    'winning_amount': float(winning_amount),
                    'status': 'sold',
                    'winner_selected_at': datetime.now()
                }
            }
        )
        ...
        purchase = {
            'user_id': ObjectId(winner_user_id),
        ...
        return jsonify({'message': 'Winner selected successfully'}), 200

    except Exception as e:
        print(f"Error selecting winner: {str(e)}")
        return jsonify({'error': f'Failed to select winner: {str(e)}'}), 500
...
def get_user_purchases():
    try:
        ...
        # Get user's purchases with item information
        pipeline = [
            {
        ...
        purchases_collection = db['purchases']
        purchases = list(purchases_collection.aggregate(pipeline))
        ...
        for purchase in purchases:
            result.append({
                ...
                'status': purchase.get('status', 'purchased'),
                'item': {
                    '_id': str(purchase['item']['_id']),
                    'name': purchase['item']['name'],
                    'description': purchase['item']['description'],
                    'image_url': purchase['item'].get('image_url'),
                    'category': purchase['item'].get('category', ''),
                    'condition': purchase['item'].get('condition', '')
                }
            })
        ...
        return jsonify({'error': f'Failed to get purchases: {str(e)}'}), 500



@app.route('/api/admin/simple-test')
def admin_simple_test():
    print("=== ADMIN SIMPLE TEST ENDPOINT CALLED ===")
    return jsonify({"message": "Admin simple test endpoint working"})
...
def admin_test():
    ...
    user_id = get_jwt_identity()
    print(f"User ID: {user_id}")
    return jsonify({"message": "Admin test endpoint working", "user_id": user_id})
...
if __name__ == '__main__':
    print("Starting Flask app with manual reload...")
